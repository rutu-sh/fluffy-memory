...
fn main() {
    ...
    println!("cosine-sim_rayon duration: {:?}", duration);


    if is_x86_feature_detected!("avx2") {

        println!("running avx2-new");
        let start_time = Instant::now();
        ...
        unsafe {
            let res = similarity::cosine_sim_avx2_new(&v1.unit_vec, &v2.unit_vec);
            println!("res = {}", res);
        }

        let duration = start_time.elapsed();

        println!("avx2-new duration: {:?}", duration);
    } 
